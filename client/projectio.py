...
def modify_goal(goal_name, goal_desc, start_date, end_date, time_spent, completion, goal_id):
    sqlite_insert_with_param = "UPDATE Goal SET GoalName = ?, GoalDesc = ?, StartDate = ?, EndDate = ?, TimeSpent = ?, Completion = ? WHERE GoalID = ? ;"
    data_tuple = (goal_name, goal_desc, start_date, end_date, time_spent, completion, goal_id)
